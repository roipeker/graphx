...
// }
  ...
  /// is no current drag object, sets the current drag bounds to null and returns.
  /// Calculates the new position of the drag object based on the mouse input
  /// event and the drag center offset, and clamps the position within the
  /// drag bounds, if provided. Finally, sets the new position of the drag object
  ...
  /// Stops the current drag operation, if this DisplayObject is currently being dragged.
  /// Removes the mouse move event listener and sets the current drag object to null.
  /// Does nothing if this DisplayObject is not currently being dragged.
  void stopDrag() {
  ...
  /// display object if it has a touchable area and [mouseEnabled] is set to true.
  ///
  ...
  /// dispatches a [MouseInputType.over] event to this display object. If the input
  /// was already captured, it checks if the mouse is still over this display
  /// object and dispatches a [MouseInputType.out] event if it's not.
  ///
  ...
  ///
  /// Depending on the type of mouse event, the appropriate callback function will be invoked.
  /// The `mouseInput` argument is a cloned copy of the `input` argument, with its `target` and `currentTarget` fields set to `this` and `object`, respectively.
  /// If the mouse event is a `down` event, the `mouseDownObj` field of this object will be set to `object`.
  /// If the mouse event is an `up` event, the `mouseDownObj` field will be set to `null`, and the appropriate `onMouseClick` or `onMouseDoubleClick` callback function will be invoked if necessary.
  ///
  /// If the mouse event is a `over` event, the `mouseOverObj` field of this object will be set to `object`.
  /// If the `useCursor` field of this object is `true` and the cursor is showing, the cursor will be set to the click cursor.
  ///
  /// If the mouse event is an `out` event, the `mouseOverObj` field will be set to `null`.
  /// If the `useCursor` field of this object is `true` and the cursor is showing, the cursor will be set to the basic cursor.
  ///
...
//  GxRect _cachedBounds;
  ...
  /// object.
  /// The mouse position is updated every frame, so this method returns the current mouse
  /// position relative to the top-left corner of this object.
  ///
  ...
  /// For example, an [Alignment] of (-1.0, -1.0) represents the top left corner,
  /// while an [Alignment] of (1.0, 1.0) represents the bottom right corner.
  ...
  /// This method is useful to adjust the pivot point to a more suitable location
  /// before performing rotations or scaling operations.
  ...
  /// Returns the bounds of this object in the coordinate space of [targetSpace].
  ///
  /// If [out] is non-null, the resulting bounds will be stored in that object
  /// and returned, otherwise a new [GRect] object will be created and returned.
  ...
  /// and returned, otherwise a new [GPoint] object will be created and returned.
  GPoint globalToLocal(GPoint globalPoint, [GPoint? out]) {
    ...
    // Transform the point using the inverted matrix to get the local coordinates.
  ...
  ///
  /// If the object has a [maskRect], this method determines whether the point is inside
  /// the rectangle.
  ///
  /// If the object has a [$mask], this method transforms the point into the local
  /// coordinate system of the mask using the [$mask]'s transformation matrix and then
  /// checks if the point is inside the mask's boundary.
  /// If [maskInverted] is `true`, the result is inverted.
  ...
  ///
  /// If [useShape] is `false`, this method uses the axis-aligned bounding box of the
  /// display object to test for a hit. If [useShape] is `true`, this method uses the
  /// object's [shape] to test for a hit.
  ///
  ...
  ///
  /// If [useShape] is `true`, the object will be tested against its shape rather
  /// than its bounding box. Defaults to `false`.
...
//    _stage = null;
  ...
  /// Creates an image representation of the current state of the [GDisplayObject].
  ///
  /// If [adjustOffset] is true, the resulting image will be offset so that its
  /// top-left corner corresponds to the top-left corner of the current [bounds].
  /// If [resolution] is provided and is different than 1, the image will be scaled by
  /// this factor before being returned.
  /// If [rect] is provided, it defines a rectangle that will be used as the bounds
  /// for the image instead of the [getFilterBounds] of the [GDisplayObject].
  /// Returns a Future that resolves to a [ui.Image] representing the current state of the
  /// [GDisplayObject] in the form of an image.
  ...
  /// which means that the method will use the filter bounds of the display object.
  /// Returns a [GTexture] instance.
