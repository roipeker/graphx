...
/// A display object representing a text element that can be added to the
/// display list.
///
/// Use [GText] to display a simple, single-line, static text element on the
/// stage.
///
/// You can specify the text content, text style, and paragraph style, and the
/// width of the text element to control the text layout. By default, the
/// [GText] uses a system font for the specified font family.
///
...
class GText extends GDisplayObject {
  ...
  // The maximum text width for web.
  static const double _maxTextWidthForWeb = 10000;

  // The rendered paragraph object for this instance.
  ui.Paragraph? _paragraph;

  // Signal to indicate when the font is loaded to rebuild the [GText].
  Signal? _onFontLoaded;

  /// The paint object used to control the alpha channel of the text.
  ...
  // The paragraph builder used for constructing the text.
  late ui.ParagraphBuilder _builder;

  // The paragraph style used for rendering the text.
  ui.ParagraphStyle? _paragraphStyle;

  // Flag indicating if the text size needs to be recalculated.
  bool _invalidSize = true;

  // Flag indicating if the paragraph builder needs to be recreated.
  ...
  /// Set the [alpha] value of this object and update the alpha value of the
  /// internal paint object that is used to render the text with the specified
  /// opacity.
  @override
  ...
  /// information. The [ui.Paragraph] instance is created by the
  /// [_invalidateBuilder] method when the text or its style is updated. The
  /// [_layout] method then lays out the text in the paragraph, updating its
  /// size.
  ///
...
//  }
  ...
  /// The [width] of the text. If not explicitly set, it returns
  /// [double.infinity].
  ///
  ...
  ///
  /// If the text's [alpha] is less than 1.0, the text is drawn with an alpha
  /// mask to prevent color bleeding. Otherwise, the text is drawn directly.
  @override
...
//        var bounds = Rect.fromLTWH(0, 0, textWidth, textHeight);
  ...
  ///
  /// The [targetSpace] parameter determines the coordinate space of the
  /// returned bounds object. You can use this parameter to get the bounds of
  /// the text relative to any display object.
  ///
  /// If the [targetSpace] parameter is not passed, the method returns the
  /// bounds of the text in its local coordinate space.
  ///
  ...
  ///
  /// If the provided [style] is the same as the current style, nothing is done.
  /// Otherwise, the [style] is set and the paragraph builder is invalidated.
  ...
  /// Callback function to be invoked when the font is loaded.
  /// This invalidates the paragraph builder and dispatches the [onFontLoaded]
  /// signal.
  void _fontLoaded() {
    _invalidBuilder = true;
    _invalidateBuilder();
  ...
  /// The paragraph, based on the [width] of the [GText] and its paragraph
  /// constraints.
  ///
  /// The paragraph width is defined by [_width], which can be set by the user
  /// or calculated automatically based on the intrinsic width of the text. The
  /// [_maxTextWidthForWeb] constant is used as a maximum value for the
  /// paragraph width when the text is rendered on the web, where infinite
  /// widths may cause issues.
  ///
  ...
  /// display object container to which the text will be added. The [width]
  /// argument is the width of the text box, which defaults to double.infinity.
  /// If the [textAlign] argument is not [ui.TextAlign.left], the [width]
  ...
  ///
  /// Throws an exception if [textAlign] is not [ui.TextAlign.left] and [width]
  /// is not specified.
  static GText build({
    String? text,
